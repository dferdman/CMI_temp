...
class PATHS:
    ...
    @classmethod
    def chdir(cls, *,main_dir = 'C:\\Users\\DF\\Documents\\'):
...
train_data = (
    train_series
    .clone()
    .filter(pl.col('series_id').is_in(test_series.clone().select('series_id').unique().collect()['series_id'].to_list()))
    .collect()
    )
